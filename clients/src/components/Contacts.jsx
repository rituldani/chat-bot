      ...
      <div className='flex flex-col -space-y-1 overflow-y-scroll scrollbar-hide h-[87vh] pb-10'>
        {
          chats?.length > 0 ? chats?.map((e) => {
            return (
              <div onClick={() => {
              ...
              }} key={e._id} className={`flex items-center justify-between sm:gap-x-1 md:gap-x-1 mt-5 ${activeChat._id === e._id ? "bg-[#fafafa] dark:bg-zinc-600" : "bg-[#fff] dark:bg-zinc-700"} cursor-pointer  py-4 px-2`}>
                <div className='flex items-center gap-x-3 sm:gap-x-1 md:gap-x-3'>
                  ...
                  <div>
                    <h5 className='text-[13.6px] sm:text-[16px] text-[#2b2e33] dark:text-[#E5E7EB] font-bold'>{getChatName(e, activeUser)}</h5>
                    <p className='text-[13.6px] sm:text-[13.5px] font-medium text-[#56585c] dark:text-[#b0b3b8] '>  {e.latestMessage?.message.length > 30
